...
% T_205 - S-function for a tank model simulating volume and concentration dynamics.
% Inputs:
% t - Time
% x - State variables
% u - Input variables
...
function sys = mdlDerivatives(t,x,u)
    ...
    C_X_tank = x(2);      % State 2: Concentration in the tank (g/L)

    % Inputs
    Q_T = u(1);           % Input 1: Input flow rate (L/h)
    C_X_T = u(2);         % Input 2: Input concentration (g/L)
    Q_OUT = u(3);         % Input 3: Output flow rate (L/h)

    % Derivatives
    dVdt = Q_T - Q_OUT;  % Rate of change of tank volume (L/h)
    dC_X_dt = (C_X_T * Q_T - C_X_tank * Q_OUT - C_X_tank * (Q_T - Q_OUT)) / V; % Rate of change of concentration (g/L/h)
...
function sys = mdlOutputs(t,x,u)
    ...
    % C_X_tank: Concentration in the tank
    sys = [V; C_X_tank];  % Return outputs: Volume and concentration
