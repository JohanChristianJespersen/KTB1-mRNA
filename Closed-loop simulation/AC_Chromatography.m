function [sys,x0,str,ts,simStateCompliance] = AC_Chromatography(t,x,u,flag,gridsize)
...
par = struct(...
    ...
    'epsbed',  0.35,...                             % no unit
    'epspar',  0.8,...                              % no unit
    'rad', 250E-6,...                               %  converteret til [dm]
    'keff',  [0.00870, 0.02273, 0.00990],...        % from cm/s converted to [dm/h] 
   ...
     'ads', [3.6E-10, 3.6E-7, 0.031065],...             % Langmuir adsorption coefficient [dm^3/g/h]
     'des', [1.44E-13, 3.6E-10, 5],...           % Langmuir desorption coefficient [1/h]
     'qmax', [1.44E-6, 1.02E-6, 3]);                % Langmuir maximum concentration [g/dm^3]
...
for j = 1:par.components
    ...
    sum =1 - (c(:,3,1) / par.qmax(j)); 
    dcdt(:,j,3) = max(0, par.ads(j) * par.qmax(j) * sum .* c(:,j,2) - ...
                  par.des(j) * c(:,j,3));
